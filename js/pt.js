...
/*
 Add by med^mandarine
 Get Audio Transport
 */
Protracker.prototype.getTransportAudio = function () {
  var ctx = null,
    usingWebAudio = true;
  if (typeof AudioContext !== "undefined") {
    ...
    ctx.createGainNode(); // dummy for ios
  } else {
    usingWebAudio = false;
    alert(
      "Web Audio API does not appear to be supported. Use Chrome 10.0+ or Safari 6.0+"
    );
  }
  return ctx;
};
...
Protracker.prototype.createContext = function () {
  this.context = this.getTransportAudio(); //Get transport // new webkitAudioContext();
  ...
  this.analyser.connect(this.context.destination); // Add analyzer here
};

Protracker.prototype.getAnalyser = function (ctx, CANVAS_WIDTH, CANVAS_HEIGHT) {
  ...
  ctx.fillStyle = "rgba(0, 136, 204, .5) ";

  for (i = 0; i < CANVAS_WIDTH; i += BAR_WIDTH) {
    var magnitude = freqByteData[i];
    magnitude = (magnitude / 256) * CANVAS_HEIGHT;
    ctx.fillRect(c * SPACER_WIDTH, CANVAS_HEIGHT, BAR_WIDTH, -magnitude);
    c++;
  }
  ...
  for (i = 0; i < fb; i++) {
    ...
    var height = CANVAS_HEIGHT * percent;
    var offset = CANVAS_HEIGHT - height - 1;
    var barWidth = CANVAS_WIDTH / this.analyser.frequencyBinCount;
...
Protracker.prototype.load = function (url) {
  ...
  if (asset.onProgress)
    // Add from cyril pereira
    request.onprogress = asset.onProgress;
...
Protracker.prototype.parse = function () {
  ...
  this.signatureXM = "";
  for (i = 0; i < 16; i++)
    this.signatureXM += String.fromCharCode(this.buffer[i]);

  if (this.signatureXM == "Extended Module:") this.signature = "XM";

  if (this.signatureXM.indexOf("IMPM") != -1) this.signature = "IT";
...
Protracker.prototype.mix = function (ape, b) {
  ...
  var mod =
    ape.srcElement != undefined
      ? ape.srcElement.module
      : ape.originalTarget.module;
  var outp = [];

  var bufs = [
    ape.outputBuffer.getChannelData(0),
    ape.outputBuffer.getChannelData(1),
  ];
      ...
      for (var ch = 0; ch < mod.channels; ch++) {
        ...
        if (
          ...
          mod.channel[ch].samplespeed =
            (((mod.palclock ? 7093789.2 : 7159090.5) /
              (mod.channel[ch].voiceperiod * 2)) *
              mod.finetunetable[
...
Protracker.prototype.effect_t0_4 = function (mod, ch) {
  ...
  mod.channel[ch].voiceperiod +=
    (mod.channel[ch].vibratodepth / 32) *
    mod.channel[ch].semitone *
...
Protracker.prototype.effect_t1_4 = function (mod, ch) {
  // 4 vibrato
  mod.channel[ch].voiceperiod +=
    (mod.channel[ch].vibratodepth / 16) *
    mod.channel[ch].semitone *
    (mod.vibratotable[mod.channel[ch].vibratowave & 3][
      mod.channel[ch].vibratopos
    ] /
