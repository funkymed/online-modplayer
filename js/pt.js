...
Protracker.prototype.getTransportAudio=function()
{
  ...
  if (typeof AudioContext !== 'undefined') {
    ...
    usingWebAudio = false;
    alert('Web Audio API does not appear to be supported. Use Chrome 10.0+ or Safari 6.0+');
  }
  return ctx;
};
...
Protracker.prototype.getAnalyser = function(ctx,CANVAS_WIDTH,CANVAS_HEIGHT)
{
  ...
  ctx.fillStyle = 'rgba(0,136,204,.6) ';
...
Protracker.prototype.load = function(url)
{
  ...
  if(asset.onProgress) // Add from cyril pereira
...
Protracker.prototype.parse = function()
{
  ...
  if(this.signatureXM=='Extended Module:')
    this.signature = 'XM';

  if(this.signatureXM.indexOf('IMPM')!=-1)
    this.signature = 'IT';
